...
sequenceDiagram
APP->>SERVER: 发送授权验证信息
SERVER-->>验证服务器SERVER: 授权验证信息
验证服务器SERVER-->>SERVER: 验证通过
SERVER-->>SERVER: 生成JWT
SERVER-->>SERVER: 生成RefreshToken
SERVER->>APP: 返回JWT（包含RefreshToken
```
...
```mermaid
sequenceDiagram
APP->>SERVER: 发送授权验证信息(RefreshToken
SERVER-->>验证服务器SERVER: 授权验证信息
验证服务器SERVER-->>SERVER: 验证通过
SERVER-->>SERVER: 标记RefreshToken已使用
SERVER-->>SERVER: 生成JWT
SERVER-->>SERVER: 生成Refresh Token
SERVER->>APP: 返回JWT（包含Refresh Token
```
...
def verify_refresh_token(token):
    ...
    # 校验token 是否有效，以及是否是refresh token
